...
pub(crate) fn outer_inline(
    ...
    let (min_min_content, min_max_content, min_depends_on_block_constraints) =
        match content_min_size.inline {
            Size::Numeric(numeric) => (numeric, numeric, false),
            Size::Initial => (auto_minimum.inline, auto_minimum.inline, false),
            Size::MinContent => (
                content_size.sizes.min_content,
                content_size.sizes.min_content,
                content_size.depends_on_block_constraints,
    ...
    let (max_min_content, max_max_content, max_depends_on_block_constraints) =
        match content_max_size.inline {
            Size::Numeric(numeric) => (Some(numeric), Some(numeric), false),
            Size::Initial => (None, None, false),
            Size::MinContent => (
                Some(content_size.sizes.min_content),
                Some(content_size.sizes.min_content),
                content_size.depends_on_block_constraints,
            ),
            Size::MaxContent => (
                Some(content_size.sizes.max_content),
                Some(content_size.sizes.max_content),
                content_size.depends_on_block_constraints,
            ),
            Size::Stretch | Size::FitContent => (
